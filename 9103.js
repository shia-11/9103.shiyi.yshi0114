let scaleFactor;
let randomHueMode = false; // Global variable: toggle random color mode
...
function drawLinGroup(x, y, len, count, spacing, weight, baseAlpha = 255) {
  // Base color control logic
  let baseR, baseG, baseB;

  if (randomHueMode) {
    baseR = random(100, 255);
    baseG = random(100, 255);
    baseB = random(100, 255);
  } else {
    baseR = map(mouseX, 0, width, 50, 255);
    baseG = map(mouseY, 0, height, 50, 255);
    baseB = 180;
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Calculate distance between mouse and current line
    let d = abs(mouseY - yOffset);
    let dynamicWeight = d < 30 ? map(d, 0, 30, weight * 3, weight) : weight;

    strokeWeight(dynamicWeight);

    let alpha = map(i, 0, count - 1, 0, baseAlpha);
    let r = constrain(baseR + i * 2 + random(-10, 10), 0, 255);
    let g = constrain(baseG + i * 2 + random(-10, 10), 0, 255);
    let b = constrain(baseB + i * 2 + random(-10, 10), 0, 255);
...
function drawTrapezoidLines(x1, x2, x3, x4, y, h, spacing = 3, alpha = 255) {
  ...
  for (let i = 0; i < h; i++) {
    ...
    // Calculate the two endpoints of the current line
    let xi1 = lerp(x1, x3, i / h);
    let xi2 = lerp(x2, x4, i / h);

    // Check if mouse is near this line
    let d = dist(mouseX, mouseY, (xi1 + xi2) / 2, yi);
    let weight = d < 30 ? map(d, 0, 30, 8, 1) : 1;
    strokeWeight(weight);

    let r = constrain(baseR + random(-20, 20), 0, 255);
    let g = constrain(baseG + random(-20, 20), 0, 255);
    let b = constrain(baseB + random(-20, 20), 0, 255);
...
function keyPressed() {
  // Toggle random color mode when spacebar is pressed
  if (key === ' ') {
    randomHueMode = !randomHueMode;
  }
}