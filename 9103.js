...
function setup() {
  ...
  // Calculate scaling factor based on original design dimensions
...
function draw() {
  ...
  // Center and transform the canvas for consistent layout
  translate(width / 2, height / 2);
  scale(scaleFactor);
  rotate(radians(-30));
  translate(-430, -470);

  // Draw groups of thick lines with specified translations
  // Thick 5
  ...
  // Thin 2 - Small trapezoid line sets
  ...
  // Trapezoid 3 - Middle size trapezoid sets
  ...
  // Trapezoid 2 - Larger size trapezoid sets
  ...
  // Trapezoid 1 - Longest trapezoid sets
...
// Draw a group of horizontal lines with transparency gradient
...
// Draw a set of trapezoid-like lines with gradual interpolation
...
// Handle window resize and re-scale canvas
function windowResized() {
  resizeCanvas(windowWidth, windowHeight);
  scaleFactor = min(width / 800, height / 600);
}

// Variation using HSB color gradient
function drawLinGroup(x, y, len, count, spacing, weight, baseAlpha = 255) {
  strokeWeight(weight);
...
// Trapezoid line rendering with dynamic hue
function drawTrapezoidLines(x1, x2, x3, x4, y, h, spacing = 3, alpha = 255) {
  colorMode(HSB);
  strokeWeight(1);
  let baseHue = random(360);
